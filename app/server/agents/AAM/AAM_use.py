...
class AAM_Class:
    ...
    def trainWithOnePath(self):
        return self.train()

    def trainWithImages(self, training_images):
        aam = self.trainAAM(training_images)
        fitter = self.trainFitter(aam)
    ...
    def getLandmarksFromGrayNdArray(self, fitter, ndArray):

        result = self.getResulsFromGrayNdArray(fitter, ndArray)

        if(result == []):
            return []

        landmarks = result.final_shape.points
        return landmarks

    def getLandmarksFromColorImage(self, fitter, image):

        landmarks = self.getResulsFromColorImage(fitter, image).final_shape
        return landmarks
...
path_to_store_aam_model = '../cache_results/fitter_save'
